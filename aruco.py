...
image = cv2.imread('test_lox3.png')
frame = rotateImage(image, 5)
...
reg = re.compile('[] '
                 '[]')
id_marker = reg.sub('', str(ids) )

# print((re.search('4', id_marker)).group(0))
# print(id_marker.index('4'))
print(id_marker[id_marker.index('1')])
# print(np.extract(re.search('', str(ids)), ids))

###coord top left
# id1 = corners[4]
# reg = re.compile('[].[]')
# count = reg.sub('', str(id1[0,0]) )
